import { randInt } from '../engine/rng';
import type { Scene } from '../engine/scene';
import type { GameCtx } from '../game/context';
import { loadSettings, saveSettings } from '../game/settings';
import { partyClient } from '../net/party';
import { roundedRect } from '../ui/text';

const CODE_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
const NAME_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 -_';

export function generateRoomCode(): string {
  let code = '';
  for (let i = 0; i < 6; i++) code += CODE_CHARS[randInt(0, CODE_CHARS.length - 1)];
  return code;
}

type LobbyMode = 'menu' | 'create' | 'join' | 'joined';

export class LobbyScene implements Scene {
  mode: LobbyMode = 'menu';
  roomCode = '';
  joinInput = '';
  displayName = loadSettings().displayName;
  editingName = false;
  status = 'Create a room or join with a 6-character code.';
  autoJoinPending = false;
  private unsub: (() => void) | null = null;

  constructor(private ctx: GameCtx) {}

  enter(): void {
    this.displayName = loadSettings().displayName;
    this.unsub?.();
    this.unsub = partyClient.on((msg) => {
      if (msg.t === 'lobby') {
        this.status = lobbyStatus(msg);
        if (msg.you !== undefined && msg.you >= 0) partyClient.mySlot = msg.you;
        if (this.mode === 'join' && partyClient.connected) {
          this.mode = 'joined';
        }
      }
      if (msg.t === 'err') this.status = msg.msg;
    });

    if (this.autoJoinPending && this.joinInput.length === 6) {
      this.autoJoinPending = false;
      this.connectWithName(this.joinInput);
    }
  }

  exit(): void {
    this.unsub?.();
    this.unsub = null;
  }

  private connectWithName(code: string): void {
    saveSettings({ ...loadSettings(), displayName: this.displayName.trim() || 'Wizard' });
    this.displayName = loadSettings().displayName;
    partyClient.connect(code, this.displayName);
    history.replaceState(null, '', `/join/${code}`);
    this.status = `Joining ${code}…`;
  }

  update(_dt: number): void {
    const { input } = this.ctx;
    if (!input.wasClicked(0)) return;

    const W = this.ctx.viewport.width;
    const H = this.ctx.viewport.height;
    const cx = W / 2;
    const bw = 320;
    const bx = cx - bw / 2;

    const hit = (x: number, y: number, w: number, h: number) =>
      input.mouse.x >= x && input.mouse.x <= x + w && input.mouse.y >= y && input.mouse.y <= y + h;

    if (this.editingName) {
      for (let i = 0; i < NAME_CHARS.length; i++) {
        const col = i % 13;
        const row = Math.floor(i / 13);
        const kx = bx + col * 24;
        const ky = H * 0.5 + row * 30;
        if (hit(kx, ky, 22, 26) && this.displayName.length < 16) {
          this.displayName += NAME_CHARS[i];
        }
      }
      if (hit(bx + bw - 60, H * 0.38, 50, 32) && this.displayName.length > 0) {
        this.displayName = this.displayName.slice(0, -1);
      }
      if (hit(bx, H * 0.72, bw, 44)) {
        this.editingName = false;
        saveSettings({ ...loadSettings(), displayName: this.displayName.trim() || 'Wizard' });
        this.displayName = loadSettings().displayName;
      }
      return;
    }

    if (this.mode === 'menu') {
      if (hit(bx, H * 0.36, bw, 40)) {
        this.editingName = true;
        return;
      }
      if (hit(bx, H * 0.46, bw, 64)) {
        this.mode = 'create';
        this.roomCode = generateRoomCode();
        this.connectWithName(this.roomCode);
        this.status = `Room ${this.roomCode} — waiting for players…`;
        return;
      }
      if (hit(bx, H * 0.46 + 80, bw, 64)) {
        this.mode = 'join';
        this.joinInput = '';
        this.status = 'Enter a 6-character room code.';
        return;
      }
      if (hit(bx, H - 90, bw, 44)) {
        partyClient.disconnect();
        this.ctx.toMenu();
      }
      return;
    }

    if (this.mode === 'create' || this.mode === 'joined') {
      const readyY = H * 0.66;
      if (hit(bx, readyY, bw, 44)) {
        const me = partyClient.players.find((p) => p.slot === partyClient.mySlot);
        partyClient.setReady(!(me?.ready ?? false));
      }
      if (this.mode === 'create' && hit(bx, H * 0.74, bw, 52)) {
        const mePlayer = partyClient.players.find((p) => p.slot === partyClient.mySlot);
        if (mePlayer?.connId === partyClient.hostId) {
          partyClient.startMatch();
        }
      }
      if (this.mode === 'create' && hit(bx, H * 0.58, bw, 44)) {
        const link = `${location.origin}/join/${this.roomCode}`;
        void navigator.clipboard?.writeText(link);
        this.status = 'Invite link copied!';
      }
      if (hit(bx, H - 90, bw, 44)) {
        partyClient.disconnect();
        this.mode = 'menu';
        history.replaceState(null, '', '/');
      }
      return;
    }

    if (this.mode === 'join') {
      for (let i = 0; i < CODE_CHARS.length; i++) {
        const col = i % 9;
        const row = Math.floor(i / 9);
        const kx = bx + col * 34;
        const ky = H * 0.48 + row * 36;
        if (hit(kx, ky, 32, 32) && this.joinInput.length < 6) {
          this.joinInput += CODE_CHARS[i];
        }
      }
      if (hit(bx + bw - 70, H * 0.4, 60, 36) && this.joinInput.length > 0) {
        this.joinInput = this.joinInput.slice(0, -1);
      }
      if (hit(bx, H * 0.72, bw, 52) && this.joinInput.length === 6) {
        this.connectWithName(this.joinInput);
      }
      if (hit(bx, H - 90, bw, 44)) {
        partyClient.disconnect();
        this.mode = 'menu';
        history.replaceState(null, '', '/');
      }
    }
  }

  render(g: CanvasRenderingContext2D): void {
    const { viewport } = this.ctx;
    const W = viewport.width;
    const H = viewport.height;

    g.fillStyle = '#07070f';
    g.fillRect(0, 0, W, H);

    g.textAlign = 'center';
    g.font = '800 44px system-ui, sans-serif';
    g.fillStyle = '#fff';
    g.fillText('PLAY ONLINE', W / 2, H * 0.14);

    this.renderConnectionPill(g, W, H);

    g.font = '500 16px system-ui, sans-serif';
    g.fillStyle = 'rgba(200, 205, 245, 0.75)';
    g.fillText(this.status, W / 2, H * 0.24);

    const bw = 320;
    const bx = W / 2 - bw / 2;

    if (this.editingName) {
      g.font = '600 16px system-ui, sans-serif';
      g.fillStyle = 'rgba(200, 205, 245, 0.8)';
      g.fillText('EDIT DISPLAY NAME', W / 2, H * 0.32);
      g.font = '800 32px system-ui, sans-serif';
      g.fillStyle = '#9fe8ff';
      g.fillText(this.displayName || '_', W / 2, H * 0.4);
      drawBtn(g, bx, H * 0.72, bw, 44, 'DONE', 'max 16 characters');
      return;
    }

    drawBtn(g, bx, H * 0.36, bw, 40, `NAME: ${this.displayName}`, 'click to edit');

    if (this.mode === 'menu') {
      drawBtn(g, bx, H * 0.46, bw, 64, 'CREATE ROOM', 'get a 6-letter code');
      drawBtn(g, bx, H * 0.46 + 80, bw, 64, 'JOIN ROOM', "enter a friend's code");
    } else if (this.mode === 'create') {
      g.font = '800 56px system-ui, monospace';
      g.fillStyle = '#9fe8ff';
      g.fillText(this.roomCode, W / 2, H * 0.44);
      drawBtn(g, bx, H * 0.58, bw, 44, 'COPY INVITE LINK', '');
      this.renderSlots(g, W, H * 0.5);
      const me = partyClient.players.find((p) => p.slot === partyClient.mySlot);
      drawBtn(
        g,
        bx,
        H * 0.66,
        bw,
        44,
        me?.ready ? 'READY ✓' : 'NOT READY',
        'toggle ready status',
      );
      const canStart =
        partyClient.players.length >= 2 && partyClient.players.every((p) => p.ready);
      const mePlayer = partyClient.players.find((p) => p.slot === partyClient.mySlot);
      const isHost = mePlayer?.connId === partyClient.hostId;
      drawBtn(
        g,
        bx,
        H * 0.74,
        bw,
        52,
        'START MATCH',
        canStart && isHost ? 'start when everyone is ready' : canStart ? 'waiting for host' : 'all players must ready',
      );
    } else if (this.mode === 'join') {
      g.font = '800 40px system-ui, monospace';
      g.fillStyle = '#ffe94d';
      g.fillText(this.joinInput.padEnd(6, '_'), W / 2, H * 0.4);
      drawBtn(g, bx, H * 0.72, bw, 52, 'JOIN', this.joinInput.length === 6 ? 'connect' : 'need 6 chars');
    } else if (this.mode === 'joined') {
      g.font = '800 40px system-ui, monospace';
      g.fillStyle = '#9fe8ff';
      g.fillText(partyClient.code, W / 2, H * 0.4);
      this.renderSlots(g, W, H * 0.5);
      const me = partyClient.players.find((p) => p.slot === partyClient.mySlot);
      drawBtn(
        g,
        bx,
        H * 0.66,
        bw,
        44,
        me?.ready ? 'READY ✓' : 'NOT READY',
        'toggle ready status',
      );
      g.font = '500 14px system-ui, sans-serif';
      g.fillStyle = 'rgba(180, 190, 240, 0.6)';
      g.fillText('Waiting for host to start…', W / 2, H * 0.76);
    }

    if (partyClient.lastError) {
      g.font = '600 14px system-ui, sans-serif';
      g.fillStyle = '#ff6a5a';
      g.fillText(partyClient.lastError, W / 2, H * 0.88);
    }

    g.font = '600 16px system-ui, sans-serif';
    g.fillStyle = 'rgba(130, 140, 255, 0.9)';
    g.fillText('BACK', W / 2, H - 50);
  }

  private renderConnectionPill(g: CanvasRenderingContext2D, W: number, H: number): void {
    const status = partyClient.connectionStatus;
    const labels: Record<string, string> = {
      connected: '● Connected',
      connecting: '◌ Connecting…',
      reconnecting: '◌ Reconnecting…',
      disconnected: '○ Offline',
    };
    const colors: Record<string, string> = {
      connected: '#5dffa8',
      connecting: '#ffe94d',
      reconnecting: '#ffe94d',
      disconnected: '#888',
    };
    g.font = '600 13px system-ui, sans-serif';
    g.fillStyle = colors[status] ?? '#888';
    g.fillText(labels[status] ?? status, W / 2, H * 0.19);
  }

  private renderSlots(g: CanvasRenderingContext2D, W: number, y: number): void {
    const slots = 4;
    const sw = 72;
    const gap = 10;
    const startX = W / 2 - (slots * sw + (slots - 1) * gap) / 2;
    for (let i = 0; i < slots; i++) {
      const p = partyClient.players.find((pl) => pl.slot === i);
      const x = startX + i * (sw + gap);
      g.fillStyle = p ? 'rgba(40, 50, 120, 0.8)' : 'rgba(20, 22, 40, 0.6)';
      roundedRect(g, x, y, sw, 64, 8);
      g.fill();
      g.strokeStyle = p ? p.color : 'rgba(100, 110, 180, 0.3)';
      g.lineWidth = 2;
      roundedRect(g, x, y, sw, 64, 8);
      g.stroke();

      if (p && p.connId === partyClient.hostId) {
        g.font = '700 10px system-ui, sans-serif';
        g.fillStyle = '#ffe94d';
        g.fillText('HOST', x + sw / 2, y + 12);
      }

      g.font = '600 11px system-ui, sans-serif';
      g.fillStyle = p ? '#fff' : 'rgba(150,160,200,0.4)';
      g.fillText(p ? p.name.slice(0, 8) : 'empty', x + sw / 2, y + 32);

      if (p) {
        g.font = '600 9px system-ui, sans-serif';
        g.fillStyle = p.ready ? '#5dffa8' : 'rgba(180,190,240,0.5)';
        g.fillText(p.ready ? 'READY' : '…', x + sw / 2, y + 50);
      }
    }
  }
}

function lobbyStatus(msg: { players: { name: string; ready: boolean }[]; phase: string }): string {
  const n = msg.players.length;
  if (msg.phase !== 'lobby') return 'Match in progress…';
  const ready = msg.players.filter((p) => p.ready).length;
  return `${n}/4 players · ${ready} ready`;
}

function drawBtn(
  g: CanvasRenderingContext2D,
  x: number,
  y: number,
  w: number,
  h: number,
  label: string,
  sub: string,
): void {
  g.fillStyle = 'rgba(20, 22, 48, 0.9)';
  roundedRect(g, x, y, w, h, 10);
  g.fill();
  g.strokeStyle = 'rgba(130, 140, 255, 0.45)';
  g.lineWidth = 1.5;
  roundedRect(g, x, y, w, h, 10);
  g.stroke();
  g.textAlign = 'center';
  g.font = '800 20px system-ui, sans-serif';
  g.fillStyle = '#fff';
  g.fillText(label, x + w / 2, y + (sub ? h * 0.42 : h / 2));
  if (sub) {
    g.font = '500 13px system-ui, sans-serif';
    g.fillStyle = 'rgba(200, 205, 245, 0.65)';
    g.fillText(sub, x + w / 2, y + h * 0.72);
  }
}
