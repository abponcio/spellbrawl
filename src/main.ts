...
import { initPartyCoordinator } from './net/party-coordinator';
...
class Game implements GameCtx {
  ...
  private parseJoinRoute(): void {
    ...
    if (m) {
      ...
      if (this.lobby.joinInput.length === 6) {
        this.lobby.autoJoinPending = true;
      }
    }
  }
  ...
  endOnlineMatch(msg: Extract<ServerMessage, { t: 'match_end' }>): void {
    ...
    if (msg.stats) {
      for (const [idStr, stats] of Object.entries(msg.stats)) {
        const id = Number(idStr);
        if (!Number.isFinite(id)) continue;
        Object.assign(this.match.statsTracker.ensure(id), stats as CombatStats);
      }
    }
